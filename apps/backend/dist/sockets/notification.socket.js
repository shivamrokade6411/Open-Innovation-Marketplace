"use strict";
/*
 * Purpose: Notification socket namespace and push helpers.
 * Author: Copilot
 * Date: 2026-06-28
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.emitToUser = emitToUser;
exports.registerNotificationNamespace = registerNotificationNamespace;
const Notification_model_1 = require("../models/Notification.model");
let notificationNamespace = null;
/**
 * Emit an event to a specific user room.
 * @param userId Target user identifier.
 * @param event Event name.
 * @param data Event payload.
 * @returns void
 * @throws Never throws directly.
 */
function emitToUser(userId, event, data) {
    notificationNamespace?.to(userId).emit(event, data);
}
/**
 * Register the notification namespace.
 * @param io Socket.io server instance.
 * @returns The configured namespace.
 * @throws Error When the namespace cannot be created.
 */
function registerNotificationNamespace(io) {
    const namespace = io.of('/notifications');
    notificationNamespace = namespace;
    namespace.on('connection', async (socket) => {
        const userId = String(socket.handshake.auth?.userId ?? '');
        if (userId) {
            socket.join(userId);
            const missedNotifications = await Notification_model_1.Notification.find({ userId, isRead: false }).sort({ createdAt: -1 }).limit(20).lean();
            socket.emit('bulk_read', missedNotifications);
        }
        socket.on('notification_read', async (notificationId) => {
            await Notification_model_1.Notification.findByIdAndUpdate(notificationId, { isRead: true });
            socket.emit('notification_read', { notificationId });
        });
        socket.on('bulk_read', async () => {
            await Notification_model_1.Notification.updateMany({ userId }, { isRead: true });
            socket.emit('bulk_read', { success: true });
        });
    });
    return namespace;
}
